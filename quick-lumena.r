...
chart_Series(LumenaClose,TA="add_RSI()",name=paste("LumenaClose ",ld$"Trade Time","Last :" ,lumq[2],"Change: ",lumq[3]," ",lumq[4]))
chart_Series(LumenaReturns)
...
dev.new()
par(mfrow=c(2,1))
# these plots but very slow on daily
...
chart.TimeSeries(Return.cumulative, main="Lumena TimeSeries plot",colorset = "darkblue", legend.loc = "topleft")
...
textplot(format.df(result, na.blank=TRUE, numeric.dollar=FALSE, cdec=rep(5,dim(result)[2])), rmar = 0.8, cmar = 1)
...
par(mfrow=c(3,1))
chart_Series(pfret[,1],subset='2011::2015',name="Lumena Returns")
chart_Series(pfret[,2],subset="2011::2015",name="HSI Returns")
chart_Series(pfret[,3],subset="2011::2015",name="Portfolio with contribution Returns")
...
# 
Box.test(LumenaReturns,lag=1)
Box.test(HSIreturns,lag=1)

# Return Statistics with fancy formating
retst=table.Stats(merge(LumenaReturns,LumenaClose,HSIreturns), ci = 0.95, digits = 4)
...
SharpeRatio(LumenaReturns[drop=FALSE], Rf=.035/12)
...
SharpeRatio.annualized(LumenaReturns[drop=FALSE], Rf=.035/12)
...
lgp=12  # select algp value where the bar crosses the 95% line
...
# trying to use lattice densityplot
dev.new()
